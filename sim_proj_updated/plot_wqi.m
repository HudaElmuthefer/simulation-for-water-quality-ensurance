...
% FIX: replace the raw Arabic month text pulled from test.xlsx column A
% with generated English month-year labels. The 18 rows are known to run
% chronologically from January 2022 to June 2023 (per Section 2.3 of the
% manuscript), so this is safe and keeps the journal-required English-only
% language throughout the figure without touching the source spreadsheet.
monthStart = datetime(2022,1,1);
monthDates = monthStart + calmonths(0:n-1);
monthLabels = cellstr(datestr(monthDates, 'mmm-yyyy'));
...
for t = 1:n
    % FIX: print the English label instead of the raw Arabic month text
    fprintf('%-20s WQI = %6.1f  (%s)\n', monthLabels{t}, wqi(t), wqiClass(t));
end
fprintf('\nMean WQI = %.1f (range %.1f - %.1f)\n', mean(wqi), min(wqi), max(wqi));
% FIX: use the English month labels in the exported table too
% FIX: monthLabels is already an 18x1 column cell array (datestr/cellstr
% return one row per input date), so it must NOT be transposed here -
% doing so caused "table variables must have the same number of rows"
% against wqi' and wqiClass' (both 18x1)
wqiTable = table(monthLabels, wqi', wqiClass', 'VariableNames', {'Month','WQI','Class'});
...
ylim([0 ylimMax]);
xticks(1:n);
% FIX: English xticklabels instead of the raw Arabic month text
...
% FIX: export a clean, high-resolution copy (no MATLAB toolbar, no
% clipped edge) suitable for direct use as Figure 4 in the manuscript
exportgraphics(gcf, 'figure4_wqi.png', 'Resolution', 300);