...
outlimitscounter = mean(mc_draws, 1);  % Monte Carlo mean count per parameter
sim_sum = sum(outlimitscounter);
sim_percent = (outlimitscounter / sim_sum) * 100;

% re-index the simulated series using the REAL data's sort order (sortedIndexes
% from line 288), so both series share identical x-axis parameter labels
...
cumulativePercentages_sim = cumsum(sortedSimPercent);
tableData = [sortedParameters', num2cell(sortedOutlimitscounter'), num2cell(sortedSimPercent'), num2cell(cumulativePercentages_sim')]; 
...
%simulated pareto diagram (same parameter order as the real series)
plot( sortedSimPercent, 'red');
xlim([0,20]); 
ylim([0,20]); 
xlabel('Parameters ');
ylabel('Percentages');
legend({'Real (actual) data','Monte Carlo simulated data'}, 'Location','northeast');
title(' comparing real and simulated Pareto diagram' );
xtickangle(45);
